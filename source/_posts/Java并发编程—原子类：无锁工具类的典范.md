---
title: Java并发编程—原子类：无锁工具类的典范
author: 张权
top: false
cover: false
toc: true
mathjax: false
date: 2023-07-03 13:43:30
password:
summary: Java并发编程—原子类：无锁工具类的典范
tags:
	- Java
	- 并发编程
categories: Java并发编程
...
![](https://cdn.jsdelivr.net/gh/dendi875/images/PicGo/20230630165812.png)
...
解决 ABA 问题的思路其实很简单，增加一个版本号维度就可以了，这个和我们在[Java并发编程—StampedLock：有没有比读写锁更快的锁？](https://zhangquan.me/2023/06/30/java-bing-fa-bian-cheng-stampedlock-you-mei-you-bi-du-xie-suo-geng-kuai-de-suo/)介绍的乐观锁机制很类似，每次执行 CAS 操作，附加再更新一个版本号，只要保证版本号是递增的，那么即便 A 变成 B 之后再变回 A，版本号也不会变回来（版本号递增的）。AtomicStampedReference 实现的 CAS 方法就增加了版本号参数，方法签名如下：
...
假设线程1 运行到`WMRange or = rf.get();`停止，切换到线程2 更新了值，切换回到线程1，进入循环将永远比较失败死循环，解决方案是把对 or 的赋值移到 while 循环之内就可以了，CAS每次自旋必须要重新检查新的值才有意义。
