---
title: Java并发编程—互斥锁（下）：如何用一把锁保护多个资源？
author: 张权
top: false
cover: false
toc: true
mathjax: true
date: 2023-06-13 11:53:44
password:
summary: Java并发编程—互斥锁（下）：如何用一把锁保护多个资源？
tags:
	- Java
	- 并发编程
categories: Java并发编程
...
在[这篇文章](https://zhangquan.me/2023/06/13/java-bing-fa-bian-cheng-hu-chi-suo-shang-jie-jue-yuan-zi-xing-wen-ti/)中，我们提到**受保护资源和锁之间合理的关联关系应该是 N:1 的关系**，也就是说可以用一把锁来保护多个资源，但是不能用多把锁来保护一个资源，并且结合文中示例，我们也重点强调了“不能用多把锁来保护一个资源”这个问题。而至于如何保护多个资源，我们今天就来聊聊。
...
![](https://cdn.jsdelivr.net/gh/dendi875/images/PicGo/20230613112844.png)
...
![](https://cdn.jsdelivr.net/gh/dendi875/images/PicGo/20230613113014.png)

## 使用锁的正确姿势

在[这篇文章](https://zhangquan.me/2023/06/13/java-bing-fa-bian-cheng-hu-chi-suo-shang-jie-jue-yuan-zi-xing-wen-ti/)中，我们提到用同一把锁来保护多个资源，也就是现实世界的“包场”，那在编程领域应该怎么“包场”呢？很简单，只要我们的**锁能覆盖所有受保护资源**就可以了。在上面的例子中，this 是对象级别的锁，所以 A 对象和 B 对象都有自己的锁，如何让 A 对象和 B 对象共享一把锁呢？
...
![](https://cdn.jsdelivr.net/gh/dendi875/images/PicGo/20230613114025.png)
...
答：用this.balance 和this.password 都不行。在同一个账户多线程访问时候，A线程取款进行this.balance-=amt;时候此时this.balance对应的值已经发生变换，线程B再次取款时拿到的balance对应的值并不是A线程中的，也就是说**不能把可变的对象当成一把锁**。
